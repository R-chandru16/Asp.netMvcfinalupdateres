...
namespace Asp.net_student.Services
{
    ...
    public class LoginService: LoginRepo<Users>
    {
        ...
        {
            Users user = null;
            if (_loginContext.users.Where(e => e.Email== users.Email && e.Password==users.Password).Any())
            {
                user = _loginContext.users.FirstOrDefault(e => e.Email== users.Email && e.Password == users.Password);
                return true;
            }
